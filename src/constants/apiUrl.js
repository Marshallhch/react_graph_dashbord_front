const REST_COUNTRIES_API_URL =
  'https://restcountries.com/v3.1/all?fields=name,flags,languages';

const GET_VISITORS_API_URL = 'http://localhost:3001/visitors';
const GET_CUSTOMERS_API_URL = 'http://localhost:3001/customers';
const GET_REVENUE_API_URL = 'http://localhost:3001/revenue';
const GET_TARGET_REALITY_API_URL = 'http://localhost:3001/targetReality';
const GET_TOP_PRODUCTS_API_URL = 'http://localhost:3001/topProducts';
const GET_SALES_MAP_API_URL = 'http://localhost:3001/salesMap';
const GET_VOLUME_SERVICES_API_URL = 'http://localhost:3001/volumeServices';

export {
  GET_CUSTOMERS_API_URL,
  GET_VISITORS_API_URL,
  REST_COUNTRIES_API_URL,
  GET_REVENUE_API_URL,
  GET_TARGET_REALITY_API_URL,
  GET_TOP_PRODUCTS_API_URL,
  GET_SALES_MAP_API_URL,
  GET_VOLUME_SERVICES_API_URL,
};
